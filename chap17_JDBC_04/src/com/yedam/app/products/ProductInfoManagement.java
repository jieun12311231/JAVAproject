...
public class ProductInfoManagement extends Management {
	...
	public ProductInfoManagement() {
		boolean role = selectRole();  //selectRole누군가 로그인을 해야지만 동작 관리자인지 유저인지 구분해주는 것 
		while(true) {
			menuPrint(role);			
			int menuNo = menuSelect();			
			if(menuNo == 1 && role) {  //메뉴선택 + 권한 제한 >> 관리자만 봐야하는 경우엔 role을 넣어주면 됨
				//제품정보등록
				insertProductInfo();
				...
				//제품정보삭제
				deleteProductInfo();
			}else if(menuNo == 4) {  //둘다 볼수있는것은 번호만 적으면 됨/
				//제품정보조회
				selectOne();
			}else if(menuNo == 5) {
				//전체제품조회
	...
	private void deleteProductInfo() {
		...
		//제품정보 검색
		//-1 products 테이블에서 정보 물건이 있는지 확인
		Product product = pDAO.selectOne(productName);
		
		if(product == null) {  //물건이 없으면 실행
			System.out.println("등록된 정보가 없습니다.");
			return;
		}
		//-2 입고내역에서 검색  //값을 담아서 다른 DAO에 넣어야함.
