...
///////// 클래스 테스트
		// 배열선언 배열을 이루는 타입 지정
		// 보통은 이 방법을 많이 사용함. 배열이 가질 수 없는 값은 없음.배열의 배열도 값으로 가질 수 있음
		int[] intAry = null; // ☆★☆★
		// 단순히 인덱스를 통해서 접근하려고 해도 객체가 없으면 접근하기 힘듬 null이면 값이 없기때문에 아무 것도 할 수 없음.
		// 동작을 할 수 있는 건없는데 명령만 내리는것임.

//		int intArt2[];
//		System.out.println("intAry2[0] : "+intAry2[0]); 
		// 변수 쓰고 []안에 접근하고자 하는 인덱스의 값을 적으면 됨.
		// 배열을 선언을 하게되면 실제로는 힙영역에 객체가 생성>> 내부에 공간이 생김 >> 번지수가 들어감.

///////// scores라는 int배열 선언
		// 값을 초기화 하면서 배열을 생성한 것 //메모리 상에는 객체가 생기고 안에 칸이 3칸이 생겨서 83 90 87값을 갖고있음
